...
##Function function askForContinue() {
    read ANS
    case $ANS in
        n|N|no|NO)
            echo 0
...
#Set the colours you can use
    black = '\033[0;30m' white = '\033[0;37m' red = '\033[0;31m' green =
        '\033[0;32m' yellow = '\033[0;33m' blue = '\033[0;34m' magenta =
            '\033[0;35m' cyan = '\033[0;36m'

#Some pre - define variable
    SOFTWARE_PATH = ~ / Softwares MYGITHUB_DIR = ~ / bxgithub

#Keep -                                                                        \
    alive : update existing `sudo` time stamp until the script has finished.
                                                 while true;
do
  sudo - n true;
sleep 60;
kill - 0 "$$" || exit;
    ...
    / dev /
        null &
            ...
            cecho
        "<<<<<< Softwares install begin on Ubuntu 14.04 >>>>>>" $yellow cecho
        "Please connect the wifi and check the wifi! " $red

#include the flags which software to install
            ../
        softwaresInstallFlag.sh

        if[$FLAG_APT_GET_UPDATE == 1];
then cecho
    "<<<<<< Update the software, not upgrade system. >>>>>>" $yellow sudo apt -
    get update fi
...
do
    ...
    for item in ${apps};
do
  cecho "> ${item}" $magenta done read nothing
...
#sudo apt - get remove git
    if [ ! -d "git-2.13.1" ]; then
        ...
        tar - xzvf git - 2.7.3.tar.gz fi cd git - 2.13.1 chmod +
            x configure./ configure sudo apt - get install - y zlib1g -
            dev sudo apt - get install - y curl make sudo make install fi

            if[$FLAG_GIT_EMAIL_NAME_CONFIG == 1];
        then cecho
            "<<<<<< Configure git email name and editor. >>>>>> " $yellow cecho
            "<<<<<< Fix ~/.gitconfig file to your own cofiguration. "
            ">>>>>> " $green echo "#[user]
...
#git config-- global core.editor vim
                ...
                if[$FLAG_RIPGREP_INSTALL == 1];
        then cecho "<<<<<< Install ripgrep. >>>>>>" $yellow cd
            $SOFTWARE_PATH if[! - f
                              "ripgrep-0.2.9-x86_64-unknown-linux-musl.tar.gz"];
        then wget - nc - O ripgrep - 0.2.9 - x86_64 - unknown - linux -
            musl.tar.gz https
            : // github.com/BurntSushi/ripgrep/releases/download/0.2.9/ripgrep-0.2.9-x86_64-unknown-linux-musl.tar.gz
              tar -
            xzvf ripgrep - 0.2.9 - x86_64 - unknown - linux -
            musl.tar.gz sudo cp ripgrep - 0.2.9 - x86_64 - unknown - linux -
            musl / rg / usr / local / bin / fi fi

                if[$FLAG_CMAKE_INSTALL == 1];
        then cecho "<<<<<< Install cmake 3.9.0-rc2. >>>>>>" $yellow
#Ref : https: // cmake.org/
            cd $SOFTWARE_PATH if[! - f "cmake-3.9.0-rc2.tar.gz"];
        then wget - nc https
            : // cmake.org/files/v3.9/cmake-3.9.0-rc2.tar.gz
              tar -
              xvzf cmake - 3.9.0 - rc2.tar.gz fi if[! - d "cmake-3.9.0-rc2"];
        then cd cmake - 3.9.0 -
            rc2
#if the second times or more, comment the next two lines
                    ./
                configure make sudo make install fi fi

                if[$FLAG_ZSH_INSTALL == 1];
        then cecho "<<<<<< Install zsh. >>>>>>" $yellow
#Ref : http: // zhuanlan.zhihu.com/mactalk/19556676ls
#Restart and use the zsh
                    cd $SOFTWARE_PATH sudo apt -
                get install - y zsh echo
                "export PATH=$PATH:/usr/lib/git-core" >>
            ~ /.bashrc source ~ /.bashrc wget - nc https
                : // github.com/robbyrussell/oh-my-zsh/raw/master/tools/install.sh
                  // -O - | sh
                  chsh -
                  s / bin / zsh echo "Logout to use zsh" fi

                      if[$FLAG_AUTOJUMP_INSTALL == 1];
        then cecho "<<<<<< Install autojump. >>>>>>" $yellow
#Ref : https: // github.com/wting/autojump
            cd $SOFTWARE_PATH if[! - f "autojump_v21.1.2.tar.gz"];
        then wget - nc https
            : // github.com/downloads/joelthelion/autojump/autojump_v21.1.2.tar.gz
              tar -
              xzvf autojump_v21 .1.2.tar.gz cd autojump_v21 .1.2./
                  install.sh fi fi

                  if[$FLAG_CTAGS_INSTALL == 1];
        then cecho "<<<<<< Install ctags. >>>>>>" $yellow sudo apt -
            get install - y exuberant -
            ctags cd $SOFTWARE_PATH if[! - f "ctags-5.8.tar.gz"];
        then wget - nc http
            : // prdownloads.sourceforge.net/ctags/ctags-5.8.tar.gz
              tar -
              xzvf ctags - 5.8.tar.gz fi cd ctags -
              5.8./ configure make sudo make install cecho
                  "Usage: ctags-exuberant -R *" $green fi

                  if[$FLAG_GTAGS_INSTALL == 1];
        then cecho "<<<<<< Install gtags. >>>>>>" $yellow cd
            $SOFTWARE_PATH if[! - f "global-6.5.5.tar.gz"];
        then wget - nc http
            : // tamacom.com/global/global-6.5.5.tar.gz
              tar -
              xzvf global - 6.5.5.tar.gz fi cd global - 6.5.5 sudo apt -
              get install - y libsdl1 .2 - dev sudo apt - get install -
              y ncurses - bin sudo apt - get install - y libncurses5 -
              dev./ configure make sudo make install cecho
                  "Usage: gtags" $green fi

                  if[$FLAG_VIM_INSTALL == 1];
        then cecho "<<<<<< Install vim. >>>>>>" $yellow
#Ref : http: // www.vim.org/download.php
            cd $SOFTWARE_PATH if[! - d "vim"];
        then wget - nc -
            O vim.zip https : // github.com/vim/vim/archive/master.zip
                              unzip -
            o vim.zip mv vim - master vim fi sudo apt -
            get remove-- purge vim vim - runtime vim - gnome vim - tiny vim -
            common vim - gui - common sudo apt - get build - dep vim -
            gnome sudo apt - get install liblua5 .1 - dev luajit libluajit -
            5.1 python - dev ruby - dev libperl - dev libncurses5 -
            dev libgnome2 - dev libgnomeui - dev libgtk2 .0 - dev libatk1 .0 -
            dev libbonoboui2 - dev libcairo2 - dev libx11 - dev libxpm -
            dev libxt - dev - y sudo rm -
            rf / usr / local / share / vim sudo rm -
            rf / usr / bin / vim sudo mkdir / usr / include / lua5 .1 /
                include sudo mv / usr / include /
                lua5 .1 /*.h /usr/include/lua5.1/include/
...
if [ $FLAG_XTENSA_INSTALL == 1 ];then
    ...
    wget -nc https://doc-0o-3o-docs.googleusercontent.com/docs/securesc/ha0ro937gcuc7l7deffksulhg5h7mbp1/c66ev5j18rjbov5hsvkefgttllsrrrk2/1461578400000/00407590198758729148/*/ 0B5bwBE9A5dBXT0plVndobnU1dTg
            ? e = download tar - xjvf xtensa - lx06.tar.bz iscp xtensa - lx106 -
                  elf / usr / src /
                      -r fi

                      if[$FLAG_SYSTEM_INDICATOR_INSTALL == 1];
        then cecho "<<<<<< Install System Indicator. >>>>>>" $yellow cd
                $SOFTWARE_PATH sudo add -
            apt - repository ppa : indicator - multiload / stable -
                                   daily sudo apt - get update sudo apt -
                                   get install indicator - multiload fi

                                   if[$FLAG_LAUNCHERFODERS_INSTALL == 1];
        then cecho "<<<<<< Install LauncherFolders. >>>>>>" $yellow cd
                $SOFTWARE_PATH wget http
            : // unity-folders.exceptionfound.com/unity-launcher-folders_1.0.3_all.deb
              sudo dpkg -
              i unity - launcher - folders_1 .0.3_all.deb fi

#if[$FLAG_SKYPE_INSTALL == 1]; then
#cecho "<<<<<< Install Skype. >>>>>>" $yellow
#cd $SOFTWARE_PATH
#if[! - f "skype-ubuntu-precise_4.3.0.37-1_i386.deb"]; then
#wget - nc download.skype.com / linux / skype - ubuntu - precise_4 .3.0.37 -   \
    1_i386.deb
#fi
...
#fi
              ...
              if[$FLAG_SKYPE_INSTALL == 1];
        then cecho "<<<<<< Install Skype. >>>>>>" $yellow cd
            $SOFTWARE_PATH if[! - f "skypeforlinux-64-alpha.deb"];
        then wget - nc https : // go.skype.com/skypeforlinux-64-alpha.deb
                               sudo apt -
                               get install gdebi sudo gdebi skypeforlinux - 64 -
                               alpha.deb fi
#cecho "Use the Software center to install the deb since dpkg is not work!" $red
#cecho "Press any key to continue!" $green
#read nothing
#sudo dpkg - i skype - ubuntu - precise_4 .3.0.37 - 1_i386.deb
                               ...
                               if[$FLAG_GOOGLE_HOSTS_INSTALL == 1];
        then cecho "<<<<<< Install Google Hosts. >>>>>>" $yellow cecho
                   "By your own hand! Follow the bellow link!" $green cecho
                   "http://laod.cn/hosts/2016-google-hosts.html" $green cecho
                   "Press any key to continue!" $green read nothing fi

            if[$FLAG_ZEAL_FOR_SPACEMACE_INSTALL == 1];
        then cecho
            "<<<<<< Install ZEAL FOR SPACEMACS. >>>>>>" $yellow sudo add -
            apt - repository ppa : zeal - developers / ppa sudo apt -
                                   get update sudo apt - get install zeal fi

                                   if[$FLAG_LATEX_INSTALL == 1];
        then cecho "<<<<<< Install LATEX. >>>>>>" $yellow sudo apt -
            get install texlive - full
#latex editor
                                      sudo apt -
            get install texmaker fi

            if[$FLAG_SCEL2PYIM_FOR_SPACEMACE_INSTALL == 1];
        then cd $SOFTWARE_PATH cecho
            "<<<<<< Install scel2pyim for spacemacs chinese layer. "
            ">>>>>> " $yellow git clone https
            : // github.com/E-Neo/scel2pyim.git
              cd scel2pyim gcc -
              o scel2pyim scel2pyim.c sudo cp scel2pyim / usr / local / bin /
#sogou pin cell library path : / usr / share / sogou -                         \
    qimpanel / cell / defaultCell /
                  fi

                  if[$FLAG_WAKATIME_FOR_SPACEMACE_INSTALL == 1];
        then cecho "<<<<<< Install wakatime for spacemacs. >>>>>> " $yellow sudo
            pip install wakatime fi

            if[$FLAG_PANDOC_FOR_SPACEMACE_INSTALL == 1];
        then cecho
            "<<<<<< Install pandoc for spacemacs. >>>>>> " $yellow sudo apt -
            get install pandoc fi

            if[$FLAG_DASH_ZEAL_DOCSETS_FOR_SPACEMACE_INSTALL == 1];
        then cecho "<<<<<< Install zeal docsets for spacemacs. >>>>>> " $yellow
                    cd ~ /
                bxgithub / git clone https
                : // github.com/baoxianzhang/myconfigresources.git
                  cd myconfigresources chmod +
            x install.sh./ install.sh rm ~ /.local / share / Zeal / Zeal /
                docsets ln -
            s ~ / bxgithub / myconfigresources / zeal / docsets ~ /.local /
                share / Zeal / Zeal / docsets fi

                if[$FLAG_HAROOPAD_FOR_MARKDOWN_INSTALL == 1];
        then cecho "<<<<<< Install Haroopad for Markdown. >>>>>> " $yellow cd
                $SOFTWARE_PATH wget https
            : // bitbucket.org/rhiokim/haroopad-download/downloads/haroopad-v0.13.1-x64.deb
              sudo dpkg -
              i haroopad - v0 .13.1 -
              x64.deb fi

              if[$FLAG_SECURECRT7dot3_INSTALL == 1];
        then cecho
            "<<<<<< Install SecureCRT 7.3. >>>>>>" $yellow cecho
            "Please install by your own hand. Follow the link below!" $green
                cecho
            "http://blog.csdn.net/chszs/article/details/40623169" $green cecho
            "Download deb and crack in baiduyun pan" $green cecho
            "$sudo dpkg -i scrt-7.3.0-657.ubuntu13-64.x86_64.deb" $green cecho
            "$sudo perl securecrt_linux_crack.pl /usr/bin/SecureCRT" $green
                cecho "Press any key to continue!" $green read nothing fi

#if[$FLAG_SHADOWSOCKS_INSTALL == 1]; then
#cecho "<<<<<< Install Shadwsocks. >>>>>>" $yellow
#sudo apt - get install python - pip
#sudo pip install-- upgrade git +                                              \
    https: // github.com/shadowsocks/shadowsocks.git@master
#wget -                                                                        \
    c https: // github.com/jedisct1/libsodium/releases/download/1.0.16/libsodium-1.0.16.tar.gz
...
#cecho                                                                         \
    "in Import/Export restore from file add *.bak and choose auto switch" $green
...
EOF
        ...
        if
          [$FLAG_SHADOWSOCKS_INSTALL == 1];
        then cecho "<<<<<< Install shadowsocks. " $yellow echo
                   "sudo pip install shadowsocks";
        sudo - H pip install shadowsocks echo - e "\033[40;32m deploy the "
                                                  "proxy server on your remote "
                                                  "vps: server[1,2,3] \033[0m"

            SS_CFG = "/etc/shadowsocks.json" if[! - f "$SS_CFG"];
        then echo "no found shadowsocks config file: /etc/shadowsocks.json";
        sudo touch "$SS_CFG" fi sudo chmod a + w "$SS_CFG" cat > "$SS_CFG"
                                                                     << EOF {
          "server" :
              [ "server1", "server2" ],
              "server_port" : 8080,
        ...
        EOF
                ...
                echo -
            e "\033[40;32m you can start the shadowsocks server on remote vps: "
              "sudo ssserver -c /etc/shadowsocks.json -d start \033[0m"
#sudo ssserver - c $SS_CFG - d stop
#sudo ssserver - c $SS_CFG - d start
            echo -
            e "\033[40;32m you can start the shadowsocks client on your local "
              "laptop: sslocal -c /etc/shadowsocks.json \033[0m" fi;

#other softwares packagesender,
        echo "" cecho "Done， Happy Hacking!" $red echo ""

            if[$FLAG_RUST_INSTALL == 1];
        then cecho
            "<<<<<< Install Rust. >>>>>>" $yellow cd $SOFTWARE_PATH wget -
            nc https
            : // static.rust-lang.org/dist/rust-1.13.0-x86_64-unknown-linux-gnu.tar.gz
              tar -
              xzvf rust - 1.13.0 - x86_64 - unknown - linux -
              gnu.tar.gz cd rust - 1.13.0 - x86_64 - unknown - linux -
              gnu sudo./
                  install.sh cargo install racer cargo install rustfmt fi

                  if[$FLAG_SOFTLINK_GIT_TEMPLATES_INSTALL == 1];
        then cecho "<<<<<< Softlink git templates. >>>>>>" $yellow
#usage : git init
